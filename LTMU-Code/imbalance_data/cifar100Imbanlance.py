...
class Cifar100Imbanlance_forget(Dataset):
    def __init__(self, imbanlance_rate=0.1, file_path="data/cifar-100-python/", num_cls=100, transform=None,
                 train=True, forget_class=None, forget=False, input_aug=False):
        if forget and input_aug:
            transform.transforms.insert(0, SelectAugment([Flip, Color, Brightness, Sharpness, Equalize], 20))
        self.transform = transform
        assert 0.0 < imbanlance_rate < 1, "imbanlance_rate must 0.0 < p < 1"
        self.num_cls = num_cls
        self.file_path = file_path
        ...
        if train is True:
            self.data = self.produce_imbanlance_data(self.imbanlance_rate, forget, input_aug)
        else:
            self.data = self.produce_test_data(forget)
        self.x = self.data['x']
        self.y = self.data['y']
        self.targets = self.data['y']
    ...
    def produce_test_data(self, forget):
        with open(os.path.join(self.file_path,"test"), 'rb') as fo:
            dict = pickle.load(fo, encoding='bytes')
            x_test = dict[b'data'].reshape([-1, 3, 32, 32]).transpose(0, 2, 3, 1)
            y_test = dict[b'fine_labels']
        ...
        if forget:
            ...
            task_train_x = x_test[index]
            label = y_test[index]
            tem_data = task_train_x
            tem_label = label
    ...
    def produce_imbanlance_data(self, imbanlance_rate, forget, input_aug=False):
